...
export const Footer: React.FC<FooterProps> = ({ onOpenIntro, onOpenInstall }) => {
  ...
  return (
    <>
      <footer className="w-full bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border-t border-zinc-200/80 dark:border-zinc-800/80 mt-20 pt-12 pb-8 transition-colors no-print">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pb-12 border-b border-zinc-200/70 dark:border-zinc-800/70">
            ...
            <div className="md:col-span-5 space-y-4">
              ...
              <p className="text-xs text-zinc-600 dark:text-zinc-300 leading-relaxed max-w-sm">
                {t.footerDesc}
              </p>

              <button
                onClick={() => setIsPrivacyOpen(true)}
                aria-label={t.footerSecurityNote}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-zinc-100 dark:bg-zinc-800/80 hover:bg-zinc-200 dark:hover:bg-zinc-700/80 border border-zinc-200/60 dark:border-zinc-700/60 text-[11px] font-semibold text-zinc-800 dark:text-zinc-200 transition cursor-pointer"
              >
            ...
            {/* Özellik Kısayolları (h4 yerine div kullanarak hiyerarşi bozulmasını engelliyoruz) */}
            <div className="md:col-span-3 space-y-3">
              <div className="text-xs font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider">
                {t.footerColFeatures}
              </div>
              <ul className="space-y-2 text-xs text-zinc-600 dark:text-zinc-300 font-medium">
                <li>
                  <button
                    onClick={onOpenIntro}
            ...
            <div className="md:col-span-4 space-y-3">
              ...
              <ul className="space-y-2 text-xs text-zinc-600 dark:text-zinc-300">
                ...
                <li className="text-zinc-500 dark:text-zinc-400 text-[11px] leading-relaxed">
                  {lang === 'tr'
              ...
              <div className="pt-2 flex flex-wrap items-center gap-3">
                <a
                  ...
                  aria-label="Denkle GitHub Repository"
                  className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-800 dark:text-zinc-200 hover:text-teal-600 dark:hover:text-emerald-400 transition-colors"
                >
                  <GithubIcon />
                  <span>GitHub</span>
          ...
          <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-[11px] text-zinc-500 dark:text-zinc-400">
      ...
      {isPrivacyOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 sm:p-7 w-full max-w-md shadow-2xl border border-zinc-200/80 dark:border-zinc-800 relative">
            ...
            <div className="text-base font-extrabold text-zinc-900 dark:text-zinc-100 tracking-tight mb-2">
              {lang === 'tr' ? '%100 Yerel ve Gizli Mimari' : '100% On-Device & Private'}
            </div>
            
            <p className="text-xs text-zinc-700 dark:text-zinc-300 leading-relaxed space-y-2 mb-4">
              {lang === 'tr'
